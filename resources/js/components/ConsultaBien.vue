<template>
	...
	<div>
		...
		<div id="resultados-busqueda" style="display:none">
			<br>
			<center>
				<p class="lead">Resultados de busqueda para el bien: {{ NombreBien }}</p>
			</center>
			<br>
					...
					<tbody>
						<tr v-for="(categoria, index) in ArrayCategorias">
							...
							<td>
								...
								<button type="button" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg" data-original-title="Editar categoria" v-on:click="AgregarCarcateristica( categoria.ID_Categoria ,categoria.ID_Bien )"> Agregar Característica
		...
		<div class="modal-dialog modal-full-height modal-right modal-notify modal-info" role="document">
			<div class="modal-content">
				...
				<div class="modal-header">
					<h4 class="title4-long-responsive-section izq blue font-bold text-uppercase"><p class="heading lead">Agregar Caracteristicas
					</p></h4>
	...
	export default{
		...
		methods:{
			...
			getBienes: function(){ //Metodo que obtiene los bienes registrados
				//var urlGetBienes = '/inventariospa/public/nuevo_inventario/getBienes';
				var urlGetBienes = '/nuevo_inventario/getBienes';
				axios.get(route('get.bienes')).then(response =>{
			...
			BuscarCategorias: function(){
				if( this.IdBien == ""){
					...
					//var urlDatos = '/inventariospa/public/categorias-by-id/'+this.IdBien;
			...
			AgregarCarcateristica: function(idCategoria, idBien){
				...
				//var urlIncluidas  = '/inventariospa/public/get-caracteristicas-incluidas/'+idCategoria;
				...
				//var urlFaltantes = '/inventariospa/public/get-caracteristicas-faltantes/'+idCategoria+'/'+idBien;
			...
			GuardarAccion:function(){
				...
				//var urlAgregarCaracteristicas = '/inventariospa/public/edit-caracteristica-categoria/';
			...
			EditarBD:function(nombre, id){
				...
				console.log("Perro");
			  	var ArrayBienesName = [];
			  	//var urlGetBienesByName = '/inventariospa/public/buscar-by-name/'+nombre+'/'+this.IDBien_Categoria;
			  	...
			  	axios.get(urlGetBienesByName).then(response =>{
			  		...
			  		if(this.ArrayExistente.length != 0){
			  			...
			  			//var UrlEditarCaracteristica = '/inventariospa/public/editar-categoria/';
			...
			EdicionEnBD:function(nombre, id){

			  	var prubaGet = '/editar-Prueba/'+this.IDCat+"/"+this.NombreNuevo+"/"+this.NombreCategoria;
			  			axios.get(prubaGet).then(response =>{
			  				this.NotificacionSucces('Se actualizo nombre a '+this.NombreNuevo.toUpperCase()+' exitosamente.');
							this.limpiar();
			  			});
